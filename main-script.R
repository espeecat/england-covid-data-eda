...
p <- ggplot(data=df, aes(x = date, y = `R value`))+geom_point() 
...
# Total Daily England 
dfbyLA <- data[data$areaName == 'Brighton and Hove',]
...
colnames(df)<-c('date','R value')
title <-paste('Estimated R value for',area)
p <- ggplot(data=df, aes(x = date, y = `R value`))+geom_point()+ggtitle(title)+theme(title =element_text(size=10 ),   axis.text.x = element_text(color = "grey20", size = 8, angle = 45, hjust = .5, vjust = .5, face = "plain"),
